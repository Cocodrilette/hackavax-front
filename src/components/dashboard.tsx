...
export function Dashboard() {
  const [votingTopics, setVotingTopics] = useState([
    { id: 1, title: 'Energy Price for Q3', options: ['$0.10/kWh', '$0.12/kWh', '$0.15/kWh'], votes: [5, 8, 3] },
    { id: 2, title: 'Solar Panel Installation', options: ['Yes', 'No', 'Postpone'], votes: [10, 2, 4] },
    { id: 3, title: 'Energy Distribution Plan', options: ['Plan A', 'Plan B', 'Plan C'], votes: [6, 7, 3] },
  ])

  const [newTopic, setNewTopic] = useState({ title: '', options: ['', '', ''] })

  const handleVote = (topicId, optionIndex) => {
    setVotingTopics(topics =>
      topics.map(topic =>
        topic.id === topicId
          ? { ...topic, votes: topic.votes.map((v, i) => i === optionIndex ? v + 1 : v) }
          : topic
      )
    )
  }
  ...
  const handleAddTopic = () => {
    if (newTopic.title && newTopic.options.every(option => option.trim() !== '')) {
      setVotingTopics([...votingTopics, { id: Date.now(), ...newTopic, votes: [0, 0, 0] }])
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Avg. Daily Production</CardTitle>
        ...
        <TabsContent value="voting" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Cast your vote on community decisions</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Propose a new topic for community voting</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <div key={index} className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewTopic({
                        ...newTopic,
                        options: newTopic.options.map((opt, i) => i === index ? e.target.value : opt)
                      })}
                    />
                  </div>
                ))}
